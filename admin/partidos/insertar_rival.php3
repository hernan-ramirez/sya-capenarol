...
$addStmt = "INSERT INTO jugadoresr (numero,nombre,partido_id,orden) values ('%s','%s','%s','%s')";

for ($x=1;$x<15;$x++){
	if($jugador[$x]<>""){
		if (!mysql_query(sprintf($addStmt,$numero[$x],$jugador[$x],$partido,$orden[$x]),$link)) {
			DisplayErrMsg(sprintf("Error ejecutando el query %s", $addStmt));
			DisplayErrMsg(sprintf("error:%d %s", mysql_errno($link), mysql_error($link)));
			exit();
		}
		$jugador[$x] = mysql_insert_id($link);
	}
	if ($cambio[$x]<>""){
		if (!mysql_query(sprintf($addStmt,0,$cambio[$x],$partido,0),$link)) {
			DisplayErrMsg(sprintf("Error ejecutando el query %s", $addStmt));
			DisplayErrMsg(sprintf("error:%d %s", mysql_errno($link), mysql_error($link)));
			exit();
		}
		$cambio[$x] = mysql_insert_id($link);
	}
?>
...
$amarillas[14] = $amarillas14;
	...
	$addStmt = "INSERT INTO amonestadosr (minuto,jugador_id,partido_id,campeonato) values ('%s','%s','%s','%s')";
for ($x=1;$x<15;$x++){
	if ($amarillas[$x]<>""){
...
$rojas[14] = $rojas14;
	...
	$addStmt = "INSERT INTO expulsadosr (minuto,jugador_id,partido_id,campeonato) values ('%s','%s','%s','%s')";
for ($x=1;$x<15;$x++){
	if ($rojas[$x]<>""){
...
$addStmt = "INSERT INTO golesrivales (partido_id,minuto,jugador_id,campeonato) VALUES ('%s','%s','%s','%s')";
$x=1;
while ($x<15){
	$y=1;
	while ($y<6){
...
$addStmt = "INSERT INTO cambiosr (minuto,sale_id,entra_id,partido_id,campeonato) values ('%s','%s','%s','%s','%s')";
