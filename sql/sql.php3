...
{
    $goto = "sql.php3?server=$server&db=$db&table=$table&pos=$pos&sql_query=".urlencode($sql_query);
}
...
else
{
    ...
    else
    {
        ...
        if(!eregi("SHOW VARIABLES|SHOW PROCESSLIST|SHOW STATUS", $sql_query))
            echo "<p><a href=\"tbl_change.php3?server=$server&db=$db&table=$table&pos=$pos&goto=$goto&sql_query=".urlencode($sql_query)."\"> $strInsertNewRow</a></p>";
    }
}
