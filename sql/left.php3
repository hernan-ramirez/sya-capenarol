...
if($server > 0)
{
    ...
    for($j=0; $j<$num_tables; $j++)
    {
        ...
        ?>
            <nobr>&nbsp;&nbsp;&nbsp;&nbsp;<a target="phpmain" href="sql.php3?server=<?php echo $server;?>&db=<?php echo $db;?>&table=<?php echo urlencode($table);?>&sql_query=<?php echo urlencode("SELECT * FROM $table");?>&pos=0&goto=tbl_properties.php3"><img src="images/browse.gif" border="0" alt="<?php echo $strBrowse.": ".$table;?>"></a>&nbsp;<a class="item" target="phpmain" HREF="tbl_properties.php3?server=<?php echo $server;?>&db=<?php echo $db;?>&table=<?php echo urlencode($table);?>"><?php echo $table;?></a></nobr><br>
        <?php
    }
